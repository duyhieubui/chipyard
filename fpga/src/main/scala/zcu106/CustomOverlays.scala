...
// FIXME: this is currently mapping into the programmable clock
...
case object ZCU106DDR4Size extends Field[BigInt](0x40000000L * 4) // 4GB
class DDR4ZCU106PlacedOverlay(val shell: ZCU106FPGATestHarness, name: String, val designInput: DDRDesignInput, val shellInput: DDRShellInput)
  extends DDRPlacedOverlay[XilinxZCU106MIGPads](name, designInput, shellInput)
{
  ...
  val ddrUI     = shell { ClockSourceNode(freqMHz = 200) }         // FIXME: 200MHz?
  ...
  shell { InModuleBody {
    ...
    // adr[0->16], we_n, cas_n, ras_n, bg, ba[0->1], reset_n, act_n, ck_c, ck_t, cke, cs_n, odt, dq[0->63], dqs_c[0->7], dqs_t[0->7], dm_dbi_n[0->7]
    val allddrpins = Seq(
      "AK9", "AG11", "AJ10", "AL8", "AK10", "AH8", "AJ9", "AG8", "AH9", "AG10", "AH13", "AG9", "AM13", "AF8", // addr[0-13]
...
class DDR4ZCU106ShellPlacer(shell: ZCU106FPGATestHarness, val shellInput: DDRShellInput)(implicit val valName: ValName)
  extends DDRShellPlacer[ZCU106FPGATestHarness] {
  def place(designInput: DDRDesignInput) = new DDR4ZCU106PlacedOverlay(shell, valName.name, designInput, shellInput)
}
